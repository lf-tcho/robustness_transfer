...
class Regression(Criterion):
    ...
    def __call__(self, perturbed: T, outputs: T) -> T:
        outputs_, restore_type = ep.astensor_(outputs)
        del perturbed, outputs
        outputs_ = ep.reshape(outputs_, [-1])
        diff = ep.square((outputs_-self.labels))
        assert diff.shape == self.labels.shape
        is_adv = diff > 2000
        return restore_type(is_adv)
    
class NormDiff(Criterion):
    """Considers those perturbed inputs adversarial whose predicted class
    differs from the label.
    Args:
        labels: Tensor with labels of the unperturbed inputs ``(batch,)``.
    ...
    def __call__(self, perturbed: T, outputs: T) -> T:
        outputs_, restore_type = ep.astensor_(outputs)
        del perturbed, outputs
        diff = (outputs_-self.labels)
        assert diff.shape == self.labels.shape
        dummy_idxs = random.sample(range(0, len(diff)), int(len(diff)*0.3))
        is_adv = diff[dummy_idxs]
        # is_adv = diff > 2000
        return restore_type(is_adv)